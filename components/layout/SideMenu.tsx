...
export default function SideMenu({ isOpen, onClose }: SideMenuProps) {
  ...
  return (
    <>
      {isOpen && (
        <div
          className="fixed inset-0 bg-[rgba(0,0,0,0.45)] z-200 animate-[fadeIn_0.2s_ease]"
          onClick={onClose}
        />
      )}

      <div
        className={`fixed top-0 left-0 h-screen w-75 max-w-[85vw] bg-background z-201 transition-transform duration-280 ease-in-out flex flex-col overflow-y-auto shadow-[4px_0_24px_rgba(0,0,0,0.14)] ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="flex items-center justify-end py-[0.6rem] px-3 border-b border-border shrink-0">
          <button
            className="bg-transparent border-0 cursor-pointer flex items-center justify-center text-foreground p-[0.4rem] rounded-md transition-colors duration-150 hover:text-primary"
            onClick={onClose}
        ...
        <div className="py-3 px-4 border-b border-border">
          ...
          >
            ...
            <BiChevronDown
              className={`transition-transform duration-200 shrink-0 ${showLanguages ? "rotate-180" : ""}`}
            />
          </button>
          {showLanguages && (
