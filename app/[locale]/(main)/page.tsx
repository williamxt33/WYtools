...
export default function HomePage() {
  ...
  return (
    <main className="w-full flex flex-col justify-center">
      <div className="mt-5">
        <h1 className="text-3xl sm:text-5xl font-bold tracking-tight text-foreground mb-4 mt-10 text-center">
          {t("title")}
        </h1>
        <p className="text-base sm:text-xl text-muted text-center">
          {t("subtitle")}
        </p>
      </div>
          ...
          <div key={cat.value} className="flex flex-col w-full gap-4">
            <div className="flex flex-row justify-between">
              <div className="flex flex-row gap-2.5">
                ...
                <h2 className="text-xl text-bold">{tR(`categories.${cat.value}`)}</h2>
              </div>
              <Link className="text-blue-500" href={`/tools/${cat.value}`}>
                {t("seeAll")}
              </Link>
            </div>
